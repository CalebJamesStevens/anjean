  ...
  public:
    ...
    Manifold EPA(const std::vector<Core::Vector3>& simplex,
      ...
      for (size_t iteration = 0; iteration < maxIterations; iteration++) {
        ...
        float supportDistance = dotProduct(support, closestFace.normal);
        ...
        for (const EPAFace& face : faceData) {
          bool canSeeFace = dotProduct(face.normal, support) > face.distance + epsilon;
    ...
    Core::Vector3 crossProduct(const Core::Vector3& a, const Core::Vector3& b) {
      return Core::Vector3{a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x};
    }
    ...
    void ResolveCollision(Core::PhysicsBodyState& A, Core::PhysicsBodyState& B,
                          const Manifold& manifold, float dt) {
      if (A.physicsBodyType == Core::PhysicsBodyType::Dynamic) {
        ...
        float invMassA = A.physicsBodyType == Core::PhysicsBodyType::Dynamic ? 1.0f / A.mass : 0.0f;

        float invMassB = B.physicsBodyType == Core::PhysicsBodyType::Dynamic ? 1.0f / B.mass : 0.0f;
        ...
        float percent = .8f;
        Core::Vector3 correction = normal * (manifold.depth * percent / (invMassA + invMassB));
        B.position += invMassB * correction;
        A.position -= invMassA * correction;
      }
    }
    std::vector<Core::PhysicsBodyResult> Step(float dt,
      ...
      for (size_t i = 0; i < bodies.size(); i++) {
        ...
        if (body.physicsBodyType == Core::PhysicsBodyType::Dynamic) {
          if (body.mass <= 0.0f) {
            continue;
          }
          body.force += body.mass * m_gravity;

          body.velocity += dt * body.force * (1.0f / body.mass);
        }
      }
      ...
      for (size_t i = 0; i < bodies.size(); i++) {
        ...
        for (size_t j = 0; j < bodies.size(); j++) {
          ...
          for (auto col : collisions) {
            if (bodies[col.a].id == colBody.id && bodies[col.b].id == body.id) {
              shouldContinue = true;
            }
          }
          if (shouldContinue)
          ...
          Core::Vector3 bodyCollisionPos = body.position;
          Core::Vector3 colCollisionPos = colBody.position;

          std::vector<Core::Vector3> m_vertsA{
              {body.colliders.at(0).halfExtents.x + bodyCollisionPos.x,
               body.colliders.at(0).halfExtents.y + bodyCollisionPos.y,
               body.colliders.at(0).halfExtents.z + bodyCollisionPos.z},
              {body.colliders.at(0).halfExtents.x + bodyCollisionPos.x,
               body.colliders.at(0).halfExtents.y + bodyCollisionPos.y,
               -body.colliders.at(0).halfExtents.z + bodyCollisionPos.z},
              {body.colliders.at(0).halfExtents.x + bodyCollisionPos.x,
          ...
          std::vector<Core::Vector3> m_vertsB{
              {colBody.colliders.at(0).halfExtents.x + colCollisionPos.x,
               colBody.colliders.at(0).halfExtents.y + colCollisionPos.y,
               colBody.colliders.at(0).halfExtents.z + colCollisionPos.z},
              {colBody.colliders.at(0).halfExtents.x + colCollisionPos.x,
               colBody.colliders.at(0).halfExtents.y + colCollisionPos.y,
               -colBody.colliders.at(0).halfExtents.z + colCollisionPos.z},
              {colBody.colliders.at(0).halfExtents.x + colCollisionPos.x,
          ...
          for (int iter = 0; iter < 32; iter++) {
            ...
            if (ProcessSimplex(simplex, direction)) {
              ...
              if (manifold.hasCollision) {
                collisions.push_back(Collision{.a = i, .b = j, .manifold = manifold});
      ...
      for (Collision& collision : collisions) {
        ResolveCollision(bodies[collision.a], bodies[collision.b], collision.manifold, dt);
      }
